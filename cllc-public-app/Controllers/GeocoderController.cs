...
namespace Gov.Lclb.Cllb.Public.Controllers
{
    ...
    public class GeocoderController : ControllerBase
    {
        ...
        public async Task<IActionResult>GetExternalData([FromQuery] string queryParam){
            var baseUrl = $"https://geocoder.api.gov.bc.ca/addresses.json?addressString={Uri.EscapeDataString(queryParam)}&locationDescriptor=any&maxResults=3&interpolation=adaptive&echo=true&brief=false&autoComplete=true&exactSpelling=false&setBack=0&outputSRS=4326&minScore=1&provinceCode=BC";
        ...
        public async Task<IActionResult>GetPID([FromQuery] string siteId){
            var baseUrl = $"https://geocoder.api.gov.bc.ca/parcels/pids/{Uri.EscapeDataString(siteId)}.json";
            using var httpClient = new HttpClient();
            try
            {
               string apikey =  _configuration["GEOCODER_API_KEY"];
